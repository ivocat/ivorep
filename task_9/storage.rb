...
class Storage
  ...
  def add_car_to_train(train_number, car_name, cars_to_hook, car_capacity = 100)
    cars_to_hook.times do
      @cars[car_name] = if @trains[train_number].is_a?(PassengerTrain)
                          PassengerCar.new(car_name, car_capacity)
                        else
                          CargoCar.new(car_name, car_capacity)
                        end
  ...
  def occupy_car(train_number, car_number, occupation)
    raise 'введено некорректное число' if occupation.zero?
